...
public class Spline : MonoBehaviour {
    ...
    /// <param name="t"></param>
    /// <returns></returns>
    public Vector3 GetLocationAndTengentAlongSpline(float t, out Vector3 tangent)
    {
        int index = GetNodeIndexForTime(t);
        return curves[index].GetLocationAndTangent(t - index, out tangent);
    }
    ...
    /// <summary>
    /// Returns the point on spline at distance and the tangent of spline at distance. Distance must be between 0 and spline length.
    /// </summary>
    /// <param name="d"></param>
    /// <returns></returns>
    public Vector3 GetLocationAndTangentAtDistance(float d, out Vector3 tangent)
    {
        var seekDisk = d;
        ...
        foreach (CubicBezierCurve curve in curves)
        {
            if (d > curve.Length)
            {
                d = Mathf.Max(0, d - curve.Length);
            }
            else
            {
                return curve.GetLocationAndTangentAtDistance(d, out tangent);
            }
        }

        //throw new Exception("Something went wrong with GetLocationAlongSplineAtDistance");
        var lastCurve = curves[curves.Count - 1];
        return lastCurve.GetLocationAndTangentAtDistance(lastCurve.Length, out tangent);
    }
    ...
    public Vector3 GetLocationAlongSplineAtDistance(float d) {
        ...
        var lastCurve = curves[curves.Count - 1];
        return lastCurve.GetLocationAtDistance(lastCurve.Length);
    ...
    public Vector3 GetTangentAlongSplineAtDistance(float d) {
        ...
        var lastCurve = curves[curves.Count - 1];
        return lastCurve.GetTangentAtDistance(lastCurve.Length);
