...
public class CubicBezierCurve {
    ...
    /// <param name="t"></param>
    /// <returns></returns>
    public Vector3 GetLocationAndTangent(float t, out Vector3 tangent)
    {
        if (t < 0 || t > 1)
        ...
        var invDirection = GetInverseDirection();

        tangent = (n1.position * (-omt2) +
                    n1.direction * (3 * omt2 - 2 * omt) +
                    invDirection * (-3 * t2 + 2 * t) +
                    n2.position * (t2)).normalized;

        return
            n1.position * (omt2 * omt) +
            n1.direction * (3f * omt2 * t) +
            invDirection * (3f * omt * t2) +
            n2.position * (t2 * t);
    }
    ...
    {
        var curve = getCurvePointAtDistance(d);
